...
run_exp_personal <- function(rmat, fmat, umat, profiles, sizes, gname, prob_feedback, prob_BLD, num_trials, num_cycles, gamma, b, topk, bins){
    ...
    for(cur in 1:num_trials){ # repeat 100 times
      ...
      for(t in 1:num_cycles){ #interaction length
        ...
        if(!is.null(group_choice)){
          if(gname!="mixed"){
            tmp_loss_personal[t,] <- compute_loss_indv(current_group,umat,group_choice)
            tmp_loss_personal_new[t,] <- compute_loss_indv_in_group(current_group,umat, group_choice, fmat, WU_updated[[t]])
          }else{
      ...
      if(gname!="mixed"){
        loss_personal <- loss_personal + tmp_loss_personal
        loss_personal_new <- loss_personal_new + tmp_loss_personal_new
      }
  ...
  fname <- paste0("results/personal_OUT_loss_type_",gname,"_cycles_", num_cycles, "_gamma_", gamma, "_b_", b, ".txt", sep = "")
  fname2 <- paste0("results/personal_IN_loss_type_",gname,"_cycles_", num_cycles, "_gamma_", gamma, "_b_", b, ".txt", sep = "")
  if(gname!="mixed"){
    write.table(loss_size,file = fname)
    write.table(loss_size_new,file = fname2)
  }else{
