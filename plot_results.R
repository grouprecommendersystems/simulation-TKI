...
plot_data <- function(size, scores, num_cycles){
  ...
  df <- data.frame(cycles = 1:num_cycles, compromise = score_types[i1,], 
                      compete = score_types[i2,], 
                      accommodate =score_types[i3,],
                      avoid = score_types[i4,],
                      collaborate = score_types[i5,])
